                   ...
                   (AH_LAYOUT_3_WIN==layout)?wxSUNKEN_BORDER:0 )
{
    ...
    m_pBrushBlack      = new wxBrush(*wxBLACK, wxBRUSHSTYLE_SOLID);
    m_pBrushWhite      = new wxBrush(*wxWHITE, wxBRUSHSTYLE_SOLID);
    m_pBrushRed        = new wxBrush(*wxRED  , wxBRUSHSTYLE_SOLID);
...
void CMapPane::SavePlaneConfig()
{
    ...
    if (gpGameData->m_pAtlantis)
    {
        CLand * pLand = gpGameData->m_pAtlantis->GetLand(m_SelHexX, m_SelHexY, m_SelPlane, true);
        if (pLand)
...
void CMapPane::ApplyOneColor(wxColour & cr, const char * name)
{
    ...
    if (!pBrush)
        pBrush = new wxBrush(cr, wxBRUSHSTYLE_SOLID);
    m_TerrainBrushes.push_back(pBrush);

    cr2.Set(cr.Red() >> 1, cr.Green()  >> 1, cr.Blue() >> 1);
    pBrush = new wxBrush(cr2, wxBRUSHSTYLE_CROSSDIAG_HATCH);
    m_TerrainBrushes.push_back(pBrush);
    ...
    if (m_Detail >= UNIT_DETAIL && pLand)
    {
        for (i=0; i<pLand->EdgeStructs.Count(); i++)
        {
            ...
            case EDGE_SHAPE_ROCKS:
                ...
                pPen->SetStyle(wxPENSTYLE_SHORT_DASH);
                pDC->SetPen(*pPen);
                pDC->DrawLine(x1,y1,x2,y2);
            }
        }
...
void CMapPane::DrawTaxTrade(wxDC * pDC, CLand * pLand, wxPoint * point)
{
    ...
    if (m_bAdvancedIcons)
    {
        ...
        if (pLand && (pLand->Flags&LAND_TAX))
        {
            ...
            wxPen * pen = new wxPen(*ink,1,wxPENSTYLE_SOLID);
            pDC->SetPen(*pen);
            ...
            wxPen * pen1 = new wxPen(*ink1,1,wxPENSTYLE_SOLID);
            pDC->SetPen(*pen1);
            ...
            wxPen * pen2 = new wxPen(*ink2,1,wxPENSTYLE_SOLID);
            pDC->SetPen(*pen2);
            ...
            wxPen * pen3 = new wxPen(*ink3,1,wxPENSTYLE_SOLID);
            pDC->SetPen(*pen3);
            ...
            wxPen * pen4 = new wxPen(*ink4,1,wxPENSTYLE_SOLID);
            pDC->SetPen(*pen4);
        ...
        if (pLand && (pLand->Flags&LAND_TRADE))
        {
            ...
            wxPen * pen5 = new wxPen(*ink5,1,wxPENSTYLE_SOLID);
            pDC->SetPen(*pen5);
...
void CMapPane::DrawUnits(wxDC * pDC, CLand * pLand, wxPoint * point)
{
    ...
    if (m_bAdvancedIcons)
    {
        ...
        if(m_Detail >= MILITARY_DETAIL)
        {
            ...
            {
                xc += 2;
                wxPen * pen = new wxPen(*m_pUnitColor[side],1,wxPENSTYLE_SOLID);
                pDC->SetPen(*pen);
        ...
        if((m_Detail>=TOWN_DETAIL)&&(pLand->AlarmFlags&ALARM))
        {
            ...
            {
                wxPen * pen = new wxPen(*m_pUnitColor[color],1,wxPENSTYLE_SOLID);
                pDC->SetPen(*pen);
...
void CMapPane::DrawShieldIcon(wxDC * pDC, int x, int y, wxColour pGuardColor, wxColour pDarkColor, bool large, bool mark)
{
        if (large)
        {
            ...
            wxPen * pen1 = new wxPen(pDarkColor,1,wxPENSTYLE_SOLID);;
            pDC->SetPen(*pen1);
    ...
    if (pLand)
    {
        ...
        if (AutoEmpty)
        {
            ...
            for (i=0; i<6; i++)
            {
                ...
                pLand = gpGameData->m_pAtlantis->GetLand(x, y, m_SelPlane, true);
...
void CMapPane::OnPaint(wxPaintEvent &WXUNUSED(event))
{
    ...
    pPlane   = (CPlane*)gpGameData->m_pAtlantis->m_Planes.At(m_SelPlane);
    ...
    pLand = gpGameData->m_pAtlantis->GetLand(m_SelHexX, m_SelHexY, m_SelPlane, true);
    DrawEdge(&dc, pPlane);
...
void CMapPane::DrawCoordPanes(wxDC * pDC, int mapwidth, int mapheight, CPlane * pPlane)
{
    ...
    while (true)
    {
        ...
        snprintf(buf, sizeof(buf), "%d", NormalizeHexX(nx, pPlane));
...
void CMapPane::RedrawTracksForUnit(CPlane * pPlane, CUnit * pUnit, wxDC * pDC, bool DoDrawCitiesAndWeather)
{
    ...
    for (i=0; i<(int)m_pTrackHexes->size(); i++)
    {
        ...
        pLand = gpGameData->m_pAtlantis->GetLand(HexId);
...
bool CMapPane::HexIsSelected(CLand * pLand)
{
    ...
    CPlane * pPlaneCrnt = (CPlane*)gpGameData->m_pAtlantis->m_Planes.At(m_SelPlane);
...
void CMapPane::OnMouseEvent(wxMouseEvent& event)
{
              ...
              (event.LeftDClick() &&  atol(gpConfigManager->GetConfig(SZ_SECT_COMMON, SZ_KEY_RCLICK_CENTERS))) )
    {
        ...
        GetHexNo(nx, ny, xpos, ypos);
        m_pPopupLand   = gpGameData->m_pAtlantis->GetLand(nx, ny, m_SelPlane, true);
...
//    CStruct        * pStruct;
    ...
    for (n=0; n<gpGameData->m_pAtlantis->m_Planes.Count(); n++)
    {
        ...
        {
            pLand = (CLand*)pPlane->Lands.At(i);
            if ( EvaluateBaseObjectByBoxes(pLand, Property, CompareOp, sValue, lValue, HEX_SIMPLE_FLTR_COUNT, gpGameData->m_pAtlantis.get()))
            {
...
void CMapPane::OnPopupMenuBattles(wxCommandEvent & event)
{
    ...
    for (i=0; i<gpGameData->m_pAtlantis->m_Battles.Count(); i++)
    {
        pBattle = (CBattle * )gpGameData->m_pAtlantis->m_Battles.At(i);
...
void CMapPane::OnPopupWhoMovesHere(wxCommandEvent & event)
{
    ...
    pCurPlane = (CPlane*)gpGameData->m_pAtlantis->m_Planes.At(m_SelPlane);
...
//    int  i;
    ...
    case tool_planedwn  :
        if (m_SelPlane < gpGameData->m_pAtlantis->m_Planes.Count()-1)
        {
            SavePlaneConfig();
            m_SelPlane++;
            LoadPlaneConfig();
...
bool CMapPane::IsToolActive(wxUpdateUIEvent& event)
{
    ...
    case tool_planedwn  :
        Ok = (m_SelPlane < gpGameData->m_pAtlantis->m_Planes.Count()-1);
        break;
