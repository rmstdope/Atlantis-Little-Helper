...
- `meson test -C build`'s summary line (`1/1 ... OK`) counts Meson-level `test()` targets, not the Catch2 test cases inside them - the single `parser-tests` binary bundles every test file's `TEST_CASE`s (116 cases/504 assertions as of the #44 PR - re-check via the command below, since this grows as tests are added) into one target. To see the actual Catch2 output/counts, use `meson test -C build --print-errorlogs`, read `build/meson-logs/testlog.txt`, or run `./build/parser-tests` directly.
- Two C++ lifetime-bug patterns already bit this suite once (found via AddressSanitizer while verifying issue 37's meson.build, both compiled and passed fine at `-O2` but crashed reliably at Meson's default `-O0` debug buildtype) and could recur in new tests: (1) code that stores a raw pointer into the global `gpDataHelper` (e.g. `CAtlaParser`'s pointer-taking constructor) needs an RAII guard to restore the previous value once it goes out of scope - see `ScopedDataHelper` in `tests/model_regression_tests.cpp`/`tests/parser_regression_tests.cpp` (this applies to any ad-hoc diagnostic code too, not just committed tests - a throwaway diagnostic without the guard SIGSEGV'd mid-investigation of the `MakePathRelative` bug below); (2) `CLand` holds raw (non-owning) pointers to `CUnit` objects added via `AddUnit`, so those `CUnit` locals must be declared *before* the `CLand` in the same scope (C++ destroys locals in reverse declaration order) - see the `CLand AddUnit`/`RemoveUnit`/`CalcStructsLoad` tests. If a new test crashes only under `meson test` (or `-Db_sanitize=address`) and not at `-O2`, suspect one of these two patterns first.
- `MakePathRelative` (`src/app/ahapp.cpp`) computes a relative path via character-by-character common-prefix matching between two absolute paths, then only trusts the match if it lands exactly on a `/` boundary on both sides - otherwise it rolls back to the last separator both sides actually agreed on. This rollback isn't decorative: drop it and two paths that coincidentally share characters mid-segment (e.g. a checkout directory and a target directory that merely both start with the same letter) get corrupted (`../LH2/...` instead of `../ALH2/...`), `ParseRep` fails with `ERR_FOPEN`, and since `LoadReport` only shows a message box for `ERR_INV_TURN` (see Debugging below), the report just silently fails to load. Found live during #44's review (PR #55); not covered by the automated suite since `ahapp.cpp` isn't in `shared_sources`. Don't simplify this back to a plain character walk.

### Debugging

- The assistant's sandboxed Bash tool environment has no working display (confirmed via a failed `screencapture` attempt: "could not create image from display") - this is a limitation of the tool sandbox, not evidence the navigator's own machine lacks one. Don't try to visually verify GUI behavior yourself; ask the navigator to run it and describe or screenshot the result.
- For "I ran the app and X doesn't work" reports: rule out stale state before suspecting a logic bug. This worktree can be shared with concurrent sessions/edits, so confirm the exact commit and do a full `rm -rf build && meson setup build && meson compile -C build` first; clear sccache's cache (its location is in `sccache --show-stats`'s output) if still in doubt. Once a clean build reproduces the issue, prefer adding temporary `fprintf(stderr, ...)` traces at the suspected boundary and asking the navigator to run `build/ah` directly from a Terminal (not double-clicked, so stderr is visible) over extensive static code diffing - this found the `MakePathRelative` bug above in minutes, after hours of code review had turned up nothing (the defect was a runtime data bug, invisible in any diff, since the changed code itself was correct in isolation).
- `ReportLoader::LoadReport` (`src/app/reportloader.cpp`) only shows a message box for one `ParseRep` failure code, `ERR_INV_TURN` - every other failure (`ERR_FOPEN`, etc.) fails completely silently: no dialog, no visible symptom beyond "the report didn't load, the map/unit list stayed empty." If a report load appears to silently no-op, trace the actual return code before assuming anything else (rendering, data model, config) is broken.
- Bisecting by testing different branches from *different* checkout directories can be misleading for path-relative bugs like the one above, since the absolute path of the checkout itself affects whether the bug reproduces - not just the code at that commit.
